...
import { getTransactionsRequest,getTransactionsSuccess,getTransactionsFailure,
...
export const getAllTransactions: getAllTransactionsHandler = async(dispatch,type:string,date:string,range:string,page:number,pageSize:number) => {
    ...
    try{
      if(range == 'day'){
        ...
        const {data} = await axios.get(`/transactions?day=${day}&month=${month}&year=${year}&type=${type}&page=${page}&size=${pageSize}`)
        //console.log(data)
        dispatch(getPaginationSuccess(data))
      }else if(range == 'month'){
        const [month,year] = [date.split('/')[0],date.split('/')[2]]
        const {data} = await axios.get(`/transactions?month=${month}&year=${year}&type=${type}&page=${page}&size=${pageSize}`)
        //console.log(data)
        dispatch(getPaginationSuccess(data))
      }else{
        const [year] = [date.split('/')[2]]
        const {data} = await axios.get(`/transactions?year=${year}&type=${type}&page=${page}&size=${pageSize}`)
        //console.log(data)
        dispatch(getPaginationSuccess(data))
      }
    }
